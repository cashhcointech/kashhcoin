...
// kashhcoin: optional setting to unlock wallet for block minting only;
...
// kashhcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // kashhcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // kashhcoin: sub-cent change is moved to fee
...
// kashhcoin: create coin stake transaction
...
// kashhcoin: check 'spent' consistency between wallet and txindex
// kashhcoin: fix wallet spent state according to txindex
...
// kashhcoin: disable transaction (only for coinstake)
